module Berksfiler
  ...
  class Formatter
    ...
    # generate a hash of 'name' => '[formatted Berksfile line]' for all local cookbooks
    # and cookbooks that have specified options
    def self::special_cookbook_lines
      ...
      Berksfiler.cookbook_options.each do |cb|
        @special_cookbook_lines[cb['name']] = "cookbook '#{cb['name']}', #{cb['options']}\n"
      end
      @special_cookbook_lines
    end
